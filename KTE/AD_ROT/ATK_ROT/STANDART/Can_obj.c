    ...
    case _CAN_QK :
         if    ( _r.Cfg._.CAN_QK == 0  ) dx = 0 ; // Нет CAN-управления QK .
         else
          {
            ...
            if ( bi_QK1_Mestnoe == 1 ) dx = 0 ; // блокировка CAN-Управления QK .
      #endif
          }
       break;
         //-------------
...
#else
          if ( Prg._.Gotovn == 1 ) // В Сборке Готовности не выдаем предупреждение об оперативном отключении в АР1 :
          {
            if ( ( ( (Reg_AvarMsg | Reg_AvarMsgFirst) & ~(_Av_VR1) ) | (Reg_PreduprMsg & ~(mask_predupr))
                 | (Reg_AvarMsgFirst2 | Reg_AvarMsg2) | (Reg_PreduprMsg2 & ~(mask_predupr2)) ) == (lw)r0 )
            {
                 if ( bo_Avar == 0 ) bo_canPredupr = 0 ;
                 else                bo_canPredupr = 1 ;
            }
            else bo_canPredupr = 1 ;
          }
          else // В Работе :
...
word  can_obj ( word code )
{
  ...
  //if ( tst_can_stop(_CAN0_Port) == 1 )  return 1 ;
