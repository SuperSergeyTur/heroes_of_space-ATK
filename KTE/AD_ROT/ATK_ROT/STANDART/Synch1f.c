...
void Synch ( struct Synchr *syn )
{
  ...
  register lword  lcx ;
    ...
    if ( (u)((w)(Timer1_Ovr-syn->T1_ovr)) > 0x7fff ) syn->T1_ovr = Timer1_Ovr-0x7fff ;
        //---
    ...
    { //--- Если после последнего измерения ЕПА уже был "подозрительный" СИ , и
      ...
      if ( syn->KSDT == 0 )
      {
        ax = (u)(w)(timer1 - (syn->NS2 + (syn->lTsyn>>16)) ) ;
      ...
      {      // control otsutstv.sync. ili perioda nige 30Hz.
        if ( (u)((w)(timer1-syn->NS_EPA)) >= _Tmax_setka ) syn->KSDT = 3 ;
      }
      return ;
    }
    else
    ...
    sover: lcx = syn->lTsyn + ((slw)(sw)syn->dTsyn << ( 16 - _r.K_synchr )) ;
        ...
        sx:
            ...
            ax = Hw( syn->lTsyn ) ;
            syn->Fsyn = (_MkSec(1000) *1000UL * 256UL) / ax ;
            //---
            syn->b_SIv = 1, syn->b_SIu = 1, syn->b_SIi = 1, syn->b_SIauto = 1 ;
