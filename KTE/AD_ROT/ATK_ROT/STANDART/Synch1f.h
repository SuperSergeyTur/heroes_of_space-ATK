...
/*-----------   Блок переменных для п/п синхронизации --------------*/
  ...
  struct Synchr
    {
      lword   NS2      ; // усредненный момент прихода СИ.
      lword   NS_EPA   ; // момент прихода СИ фиксируемый ЕПА.
      lword   NS_OLD   ; // момент прихода СИ, запоминаемый на случай, если лучшей точки для привязки не будет.
      lword   dt_old   ; // интервал от момента прихода СИ NS_OLD до идеальной точки привязки.
      lword   dTsyn    ; // отклонение периода при приходе СИ NS_OLD.
      lword   NS, tsyn ; // момент прихода "подозрительного" СИ.
      lword   T1_ovr   ; // запоминание времени прихода последнего СИ для возможности проверки их исчезновения.
      lword     lTsyn ;
      lword*  epa_time ;
//      word   time_out ; // запоминание времени прихода последнего СИ для возможности проверки их исчезновения.
      float  Fsyn     ; // текущая частота синхр. переведенная из периода.
