  ...
  word SIFU_Start ( void )
  {
   ...
   // pSyn = &Syn ;// Для начала привязываемся к базовой синхр., а затем,
    ...
    flgO._.NewSyn = 0 ;
        ...
        s_lbx = *Timer3;      // пока сихронизация только по одному Uba напряжению ротора
       //s_bx = Usb.NS2 ;      // пока сихронизация только по одному Uba напряжению ротора

        s_lbx += (lw)( _r.S_FazirovUgol + s_cx ) * Tpp_3syn >> 14  ;  // добавляем  фазировочный угол и угол управлени

       // TZ_sSec = s_bx  ;                // для следа

        DP_TZ  = s_lbx; //TZ_sSec << 4 ;                 // теперь переводим в обычный формат
