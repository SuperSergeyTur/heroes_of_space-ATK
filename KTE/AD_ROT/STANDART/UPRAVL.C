     ...
     { default :
       case k0 :
                 ...
                 {                               upr_label = k150 ;
                   ...
                   {        // для внешней системы регулирования -
                     ...
                     {
                   #ifdef _AD_ROT  // АТК или УПТФ - Роторный Выпрямитель .
                       upr_label = k200 ; // Не включаем ни регуляторы , ни импульсы . Просто вход в работу .
                   #else
       ...
       case k160:
                 ...
                 if ( bi_QKDT == 1 ) break ; // Если контактор ДТ не разомкнулся , выход .
                  ...
                  //-------
    #ifdef _AD_ROT
                  if ( (u)((w)(Timer1_Ovr - upr_time)) > _or.T_zaderg_pusk/2 )
                  {
                  ...
                  break;
       case k162:
                  if ( (u)((w)(Timer1_Ovr - upr_time)) > _or.T_zaderg_pusk )
    #else
                  if ( (u)((w)(Timer1_Ovr-upr_time)) > _Sec(0.5) )
    #endif
                  {
       ...
       case k200:       // Контроль команды CAN-останов.
                  ...
                  {                                    
                    if (Prg._.RegTok)
                    {
                     if (KTE2_RT_ON)
                     {
                       if (S.Alfa < _or.Alfa_minRT)
                       { 
                         trg_MinRT = 1;
                         if ((u)((w)(Timer1_fSec - Time_Min_RT)))
                         {
                           bo_canRTmin = 1;
                         }
                       }
