...
void  Init_posle_DistSbr ( void )
  {
      ...
      Av.all = 0 , Av2.all = 0 ;
#ifdef _ATK_ROT // Переделываем сброс по переднему фронту и уровню только для АТК .
...
#else // УПТФ , РС на СИФУ :
      Z_Skor = 0, ZISkor = 0, OuIchRS = _r.S_Alfa_Start, OuRegS_dop_kod = _r.S_Alfa_Start ;
#endif
...
//      RT_Str.cfg._.enable = 1; 
   ...
   #endif
  #else // УПТФ , параллельный РТ :
      ZIDN  = ZIDN1 = _or.I_Ogr ;  /* выход ЗИ-РТ. */
      OIRT  = 0  ;  /* интегратор регулятора тока */
      OIRT1 = 0  ;  /* интегратор регулятора тока */
  #endif
#else
...
void   DistSbros ( void )
{
  static  word  time_DS ;
    ...
    //---
    // Команду сброса подали ( собственную или сетевую из соседнего АТК ) :
  /*if ( bi_DS1 == 1 || KTE2_DS1 == 1 )
...
void IdIzm (void)
  {
#ifdef  Irot_ach
    asm_di();   // Чтобы не влезло левое прерывание при одновременном измерении
#endif          // токов Инвертора и Ротора для определения Срыва Инвертирования .
...
#ifdef _RVId
  if ( RVId.IDV1  > _r.Ig_Max ) mSet_AvarMsg2 ( _Av2_Id1Max ) ;
  if ( RVId.IDV2  > _r.Ig_Max ) mSet_AvarMsg2 ( _Av2_Id2Max ) ;
#endif
...
#endif
   #else // Стоянка для УПТФ :
        if ( ( Skor & 0x7fff ) <= _r.Stoyanka_Eds && Id_flt.out >= _r.Stoyanka_Tok  )
   #endif
  ...
  void  Control_Predohr ( void ) // Контроль предохранителей .
      ...
      // Для ШС4 в 12-пульсном СИФУ :
       ...
       //if ( bi_KFV_VKask == 0 )  mSet_PreduprMsg( _Pr_KFV_VKask ) ; DAN 10.07.2017 10:44
         ...
         // Контроль питания Усилителей Импульсов Выходных Каскадов осуществляется только при поданных импульсах :
      #ifdef  bi_KP_KZ
         if ( Prg._.Gotovn == 0 && flgO._.V_trig == 1 ) // Если появились ИУ для КЗ ротора
         {
           if ( bi_KP_KZ == 0 ) // Сработал контроль питания :
           {
             // и прошла выдержка :
             if ((u)((w)(Timer1_Ovr - Time_KP_KZ)) > _Sec(0.5)) mSet_AvarMsg2 ( _Av2_KP_KZ ) ;
           }
           else Time_KP_KZ = Timer1_Ovr ;
         }
         else   Time_KP_KZ = Timer1_Ovr ;
      #endif
...
// Программа измерения температуры от ОДНОГО аналогового датчика .
// Инициализация отсутствует .
// Фон очень короткий , и для того , чтобы часто не измерять , возможно вызывать раз в 10 мс :
void T_Izm_Dat1(void)
{
 ...
 //---
  ...
  mAD_Izm ( Izm_T_ach  , &Izm_T_full ) ;
  //------

  // Измерение происходит сразу , контролировать готовность не нужно :
  mIzmData_Conversion( Izm_T_full , ax , Izm_T_ach ) ;

    ax = (d)ax/(d)_AD_VOLT_DVUPOL;
    ...
    ax = ax*1000*_or.T_Scale;

    T_izm = (sw)ax ; // Переводим плавающую точку в целые , т.к. дальше смысла нет...

  // Проверка уровня температуры :
  if ( (sw)T_izm >= _or.T_Pr )
  {
    if ((u)((w)(Timer1_Ovr - time_T_izm)) > _Sec(2.0))
    {
      //03.09.2020 - сделан общий таймер для Аварии и Предупреждения , чтобы при переходе из Предупреждения в Аварию
      //             не было паузы между изменением Предупреждения на Аварию и наоборот .
      if ( (sw)T_izm >= (_or.T_Pr + 20*10) ) mSet_AvarMsg2 ( _Av2_Izm_T ) ;
      else                                   mSet_PreduprMsg (_Pr_Izm_T ) ;
    }
  }
  else time_T_izm = Timer1_Ovr ;
 //---
 #endif
 //---
  return ;
...
//-----------------------------
 ...
 word Common_avar ( void )
 {
    ...
    Mashtab_Dat () ;
#ifndef _AD_ROT // Для АД Роторного Выпрямителя не нужно (АТК AP2 или УПТФ) . Контролируется в АР1 АТК .
  ...
  //Control_Id_Max () ;
...
#ifndef _ATK_ROT // АТК - АР2 Роторный Выпрямитель . Контролируется в АР1 или в УПТФ .
    ...
    T_Izm_Dat ( 1 ) ;         // Обработка аналоговых измерений температуры каждого охладителя в каждом силовом шкафу и выдача сообщений .
    T_Izm_Dat1() ;            // Обработка аналогового измерения температуры от одного датчика .
...
//---
  ...
  #endif
 #else
    // Включаем вентиляторы в работе и при отключенном контакторе КЗ , по Пуск 0.1 , либо по Пуск 1.0 :
                                              (bi_Pusk_0_1 == 1 || bi_Pusk_1_0 == 1) && flgO._.KZ_command == 0 )
 #endif
                                             )
   {
   ...
   }                    // УПТФ - Если Пуск сняли или КЗ включился , но вентиляторы включены ,
...
#ifndef _AD_ROT
        /* Проверка отсутствия синхронизации */
...
//--- При синхр. якорного СИФУ от Силы моста, а возб.от СН380.
  ...
  {
    if ( (u)((w)(Timer1_Ovr - Syn2.T1_ovr)) >= 3 ) mSet_AvarMsg ( _Av_NoSynch2 ) ;
    ...
    // инициализируется при выборе рабочей синхронизации .
        /* Проверка периода на минимальное значение */
      if      ( (u)Tsyn2 < _r.TMIN )   mSet_AvarMsg2 ( _Av2_Tmin2 ) ;
        /* Проверка периода на максимальное значение */
      else if ( (u)Tsyn2 > _r.TMAX )   mSet_AvarMsg2 ( _Av2_Tmax2 ) ;
  }
#endif

#ifndef _ATK_ROT // В АР2 Роторном выпрямителе Тепло не считаем :
...
#else // сетевые связи АР2 с АР1 :
    // Кнопка-грибок на двери ( убрал её выделение 18.01.2022 ) :
  //if ( KTE2_AvarStop && av_label == 0 )  mSet_AvarMsg( _Av_Vnesh_Avar ) ;
  //else                                   mClr_AvarMsg( _Av_Vnesh_Avar ) ;

    // Авария платы АР1 ( убрал выделение грибка из всех аварий 18.01.2022 ) :
    if ( KTE2_Avar == 1 /*&& KTE2_AvarStop == 0*/ && av_label == 0 ) mSet_AvarMsg2 ( _Av2_KTE2 ) ;
    ...
    // Если подан Дистанционный Сброс - даём его на плату инвертора АР1 :
    if ( bi_DS1 == 1 && Av._.DistSbros == 1 ) bo_canDS1 = 1 ;
    else                                      bo_canDS1 = 0 ;
...
//          #endif
     ...
     #ifdef  _UPTF
          if ( bi_KF1 == 0 ) mSet_AvarMsg2( _Av2_KF_VKask ) ;
          if ( bi_KF2 == 0 ) mSet_AvarMsg2( _Av2_KF_VKask2 ) ;
          if ( bi_SF2 == 0 ) mSet_PreduprMsg( _Pr_SF2 ) ;
          if ( bi_TR  == 1 ) mSet_PreduprMsg( _Pr_TR ) ;
     #endif

     #ifdef _ATK_ROT
          if ( bi_Fsil_rot   == 0 )    mSet_AvarMsg2( _Av2_Fsil_rot ) ;
     #endif
...
#ifdef _UPTF
    // КЗ еще включён , не даём собрать готовность , для того , чтобы ячейку не включили и не было прямого пуска АД :
    // 31.08.2020 - Добавил контроль б/к КЗ на всякий случай :
    if ( flgO._.KZ_command == 1 || bi_Kontaktor_KZ == 1 )    mSet_AvarMsg( _Av_KZ_vkl ) ;
#endif
    ...
    //if ( Sch_Byte |= 0 )   mSet_ServiceMsg( _Srv_OZU_Isporch ) ;
     ...
     if ( bi_2P24 != 1 )   //  "КОНТРОЛЬ 2P24"
       ...
       //#ifndef _1VR_GOTOVN // Если выдача Готовности при наличии VR1 - то сообщение "Оперативное отключение" не выводим .
          ...
          if ( bi_VR1 == 1 || Prz._.VR1 == 1 )    mSet_AvarMsg( _Av_VR1 ) ;
       //#else
       }
       else
       {
          ...
          if ( Prz._.VR1 == 1 )    mSet_AvarMsg( _Av_VR1 ) ;
       //#endif
       }
       ...
       #ifdef _UPTF
                   //vmz_31012019
                   //или здесь изменить или перед переходом в Работу (иначе останется "смыканье" R/G каждые _Sec(2.0) )
                   //см. переход в Работу ниже (а там, по любому - добавить контроль Prz._.VR1 = 0)
                   if (can_ConfigReg ( _CAN_QK ) != 2)
       #endif
                                                                 Prz._.VR1 = 0 ;
                  }
          }
       #ifndef _KTE_5 // если нет разрешения вкл. КТЭ  в готовности
          if ( bi_Razresh_Vkl == 0 )  mSet_PreduprMsg( _Pr_RazreshVkl_KTE ) ;
       #endif
        }
...
#ifdef _UPTF
    //---
    // 05.02.2019 - Сбрасываем Блок Управления только в Сборке Готовности и то , если Пусков нет .
    // Для случая , когда Пуск и Питание Статора приходят одновременно , сделана разная фильтрация этих сигналов .
    // Фильтрация Пуска - 250 мс , фильтрация Питания Статора - 50 мс .
    //---
    // 23.03.2020 - При работе в сети 3-х УПТФ почему-то на "Ведомых" возникает предупреждение "Подано задание" .
    // каким образом "Ведомые" получают сигнал Пуск по сети ( или даже радиальный ) раньше радиального сигнала
    // Питание Статора , не могу понять ... Ведь для радиального Пуска есть выдержка , которая так же сработает
    // для Пуска , переданного по сети ...
    // Поэтому для "Ведомых" просто сделал обход проверки подачи Пуска раньше Питания Статора . Если агрегат
    // "Ведомый" , он не выдаст предупреждение "Подано Задание" , это должен контролировать "Ведущий" .
    //---
    // 01.09.2020 16:19 - В СИФУ-Рез стиралось сообщение об сбитом резисторе задания R100 ,
    // поэтому перед сбросом сообщения "Подано задание" проверяем , в рабочем ли мы режиме .
    if ( Isp.all == 0 )
    {
      //if ( bi_Pusk_0_1 == 0 && bi_Pusk_1_0 == 0 )
      if ( (bi_Pusk_0_1 == 0 && bi_Pusk_1_0 == 0) || _r.Cfg._.CAN_reg == 1 )
      {
         Block_Upravl = 3 ; // Блок Управления для входа в работу (рабочая метка) .
         // Сброс сообщения здесь оставить и лучше не убирать , чтобы гарантировано войти в Работу
         // Ведомыми УПТФ , не надеясь на централизованный сброс сообщений в Сборке Готовности :
...
#ifdef _UPTF
             // 18.04.2020 12:00 - Выдержка времени для того, чтобы Ведомый увидел по сети "Питание статора" Ведущего :
             if ( _r.Cfg._.CAN_reg == 0 || (_r.Cfg._.CAN_reg == 1 && ((u)((w)(Timer1_Ovr - T_zad_vedomiy)) >= 2)) )
#endif
             {
...
#else   // Чтобы после включения Контактора КМ1 можно было игнорировать все аварии , кроме потери б/к самого КМ1
        // и отключения ячейки - 1VR ( потеря 1QK ) .
        // Для этого в УПТФ не блокируем опрос дискретных сигналов при аварии 2Р24 .

        // Самоотключения в УПТФ быть не может .
        // Если отключилась ячейка статора :
          //  "КОМАНДА ОПЕРАТИВНОГО ОТКЛЮЧЕНИЯ" - Местная или Сетевая:
        if ( ( can_ConfigReg ( _CAN_QK ) == 0 && (( _r.Cfg2._.VR1 == 1 && bi_VR1 == 1 ) || ( _r.Cfg2._.VR1 == 0 && bi_QK1 == 0 ))) ||
               can_ConfigReg ( _CAN_QK ) == 2 )
          {
...
#ifdef _UPTF
    // КЗ включен - игнорируем аварии , кроме потери б/к самого КМ1 и отключения ячейки - 1VR ( потеря 1QK ) .
    if ( flgO._.KZ_command == 1 && flgO._.KZ == 1 && bi_Kontaktor_KZ == 1 ) // Если КЗ включен :
    {
        // Проверяем регистр аварий , в регистр Первых аварий еще не перекладывали :
        if ( (Reg_AvarMsg & (_Av_VR1 | _Av_KZ_noVkl) ) == (lw)r0 )
        {
           // Нет неигнорируемых аварий -> всё можно либо очищать , либо выводить Предупреждение в зависимости от Уставки :
           //---
           if ( _or.CfgO._.Avar_After_KZ == 0 ) // Нет вывода сообщений после включения КМ1 , очищаем всё :
           {                                    // предупреждения , служебные и конечно же , аварии :
               bo_Predupr = 0 ;
               //---
               Reg_AvarMsg  = 0 , Reg_PreduprMsg  = 0 , Reg_ServiceMsg  = 0 , mask_predupr  = 0 ;
               Reg_AvarMsg2 = 0 , Reg_PreduprMsg2 = 0 , Reg_ServiceMsg2 = 0 , mask_predupr2 = 0 ;
       #ifdef _TRETIY_REG
               Reg_AvarMsg3 = 0 , Reg_PreduprMsg3 = 0 , Reg_ServiceMsg3 = 0 , mask_predupr3 = 0 ;
       #endif
       #ifdef _INTERPRETER
               Reg_AvarMsg_obj = 0 , Reg_PreduprMsg_obj = 0 , Reg_ServiceMsg_obj = 0 , mask_predupr_obj = 0 ;
       #endif
           }
           else // Тест всех аварий после включения КМ1 - включаем Реле Предупреждение :
           {
               if ( ( Reg_AvarMsg | Reg_AvarMsg2
       #ifdef _TRETIY_REG
                                  | Reg_AvarMsg3
       #endif
       #ifdef _INTERPRETER
           ...
           //---
           // Нет неигнорируемых аварий -> Обход отработки аварии :
           goto end_reg_avar ;
        }
    }
#endif
...
//------- Регистрация первых аварий. -------------------
    ...
    if ( ( Reg_AvarMsgFirst | Reg_AvarMsgFirst2
                                                  ...
                                                  ) != (lw)r0 ) // Тест всех аварий.
      {
        ...
        mPort_ImpV( 0x3f ) ;
        //---
     #ifdef bo_Vkl_KZ // Если это АТК , отлючение КЗ :
        flgO._.Sploshnyak = 0;
        bo_Vkl_KZ = 0 ;
        flgO._.KZ = 0 ;
        flgO._.Prinud_com = 0;
...
#ifdef _UPTF
  // Метка для пропуска регистрации аварии после ключения КМ1 :
  end_reg_avar:
#endif
      //    Фиксация времени изменения количества сообщений для
      //  распечатки его по "Enter" в режиме "A1".
...
void GerconAC_Interrupt (void)
  {
      if ( mTest_Gercon() )
      {
                 //   В УПТФ c 2018 года , начиная с порта Южного , первой поставки на СМ3
                 // ( до этого Днепродзержинск в 2016 был на FR50N ) исторически сложилось
                 // подключение Геркона на 2В при проблемах с платой ДТ , выдающую Геркон
                 // при подаче СН -> СМ3 входила в режим программирования .
                 //   Аппаратчики плату исправили , но уже вышло много УПТФ ( в т.ч.
                 // Узбекистан НГМК , ИнГОК ПЭСС , Беларусь ) и это стандартное подключение
  #ifndef _UPTF  // Геркона на 2В принято решение оставить во всех УПТФ .
      ...
      #if !defined(_KTE_GD) && !defined(_ATK_ROT) // 08.12.2020 В АТК не снимаем ИУ без выдержки , чтобы не запутаться Инв/Выпр .
...
void GerconDC_Interrupt (void)
  {
      ...
      //-------------------
        ...
        // 25.12.2019 20:55 - В Узбекистане понадобилось защититься от токов холостого хода при подаче 6 кВ
        //                    на статор и от помех , создаваемых незашунтированными реле Заказчика на панели .
        //                    Кроме того , проявилось , как в Днепродзержинске , при подаче СН зимой
        //                    на холодный агрегат :
        // 03.07.2020 15:56 - Аппаратчики заявляют , что сейчас плата ДТ не ловит наводки от включения вентиляторов и
        //                    вообще , она защищена от наводок . Поэтому убираем костыли и контролируем и Геркон и Id-макс
        //                    всё время ...
     //if ( flgO._.trig_Pusk == 1 ) // Блокируем геркон на время выдержки Пуска , ИУ у нас нет, и мы бессильны ...
       {             // Контролируем защиту после включения вентиляторов , при подаче ИУ , чтобы не было наводок :
        mOtkl_Q1 () ;
        mOtkl_Imp( _Avar_imp ) ;  // отключение импульсов якоря
        Av._.Sdvig_imp = 1 ;
        Av._.Gercon = 1 ;
