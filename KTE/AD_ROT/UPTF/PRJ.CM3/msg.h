...
#define   _PROGRAM_     " Плавный пуск   "

// Децимальный номер схемы агрегата:
#define   _DECIMAL_     "A.656453.347-12 "
...
_x_far const struct Bits    bts[] =
  {
    ...
    { "AsPi0-инв.  " , &_or.PiS_Normal[0],  8 ,
                               { "резерв0    ",
                                 "Пуск 0,1   ",    // K8  - интерфейсное реле .
                                 "Пуск 1,0   ",    // К9  - интерфейсное реле .
                                 "Питан.стат.",    // К10 - интерфейсное реле .
                                 "резерв4    ",
                                 "резерв5    ",
                                 "резерв6    ",
                                 "резерв7    ",
...
#define  _Av_Zatyan_pusk       ( 0x00000008ul ) // _Av_CentroBegnik

#define  _Av_Stoyanka          ( 0x00000010ul )
#define  _Av_KZ_vkl            ( 0x00000020ul ) // _Av_GL1
...
#define  _Av_KZ_noVkl          ( 0x00040000ul ) // _Av_Iv_Max
...
#define  _Pr_SF2               ( 0x00100000ul )  // _Pr_OffsetAch
...
#define  _Pr_TR                ( 0x04000000ul )  // _Pr_Nzad_obryv
  ...
  //const char *const Tabl_AvarMsg[32]  =
       {
         ...
         "Iтт-макс.       ", // "Внутреннее КЗ.  ", // "Геркон/Iтт-макс.", //"Герк.перем.тока.",
         "Внешнее КЗ.     ", // "Герк.пост.тока. ",
#else
         "ВнутреннееКЗ-ШВ1", // "Герк/Iтт-максШВ1", //"Геркон ШС1,2.   ",
         "ВнутреннееКЗ-ШВ2", // "Герк/Iтт-максШВ2",//"Геркон ШС3,4.   ",
#endif
         "Затянув-ся пуск.", // "Центробежник.   ", //a1-03

         "Стоянка под ток.",
         "Запр.пуск-КЗвкл!", // "Возбудитель-1GL.",
         "E-макс.         ",
         "N-макс.         ",
...
//#endif
         ...
         "Варисторы-1AF.  ",
         "Предохр.сил.-1PR",
         "Перегрев резист.", // "Обрыв зад.4-20мА",
  ...
  //const char *const Tabl_AvarMsg2[32]  =
       {
         ...
         "Перегрев ШС.    ",
         "a2-29.",
         "Предохр.AV2-2KF.",  //"Пред.ВыхКаск-KF2",
#ifdef bi_KF_VKask2
         "Пред.ВыхКаск-KF1"
#else
         "Предохр.AV1-1KF." //"Предохр.ВыхКаск." //"Предохр.АV1-AV2.",
#endif
       },
