...
_x_far const  struct  Blok_Ustavok   _c  =
  {
   ...
   /* union Config  Cfg ; */             { _cfg_Kontaktor | _cfg_MT /*| _cfg_DZV*/ | _cfg_Fzapoln | _cfg_Vrasch_Dvig //| _cfg_Obj_prg
                                         | _cfg_Shtat_reg | _cfg_Control_ZSk | _cfg_Iv_Econom | _cfg_Ne_Revers } ,//| _cfg_Ostanov | _cfg_Dve_Zony | _cfg_CAN_reg
   ...
   /* word     S_Alfa_Start ;           */   _Grad( 120 ) ,  //_Grad(  90 ) ,

   /* word     S_Alfa_Max ;             */   _Grad( 150 ) ,  //_Grad(  90 ) ,

   /* word     S_Alfa_Min ;             */   _Grad(   5 ) ,

   /* word     S_DeltaAlfa_Max ;        */   _Grad(  20 ) ,

   /* word     S_ImpLong ;              */   _Grad(  60 ) , // не используется , задние фронты по передним следующих ИУ .
...
#ifndef _ZI_30min
   /* word     T_ZISkorP_Razg  ;        */  _Skor_nom( 0.020 ) ,

   /* word     T_ZISkorP_Torm  ;        */  _Skor_nom( 0.025 ) ,

   /* word     T_ZISkorM_Razg  ;        */  _Skor_nom( 0.020 ) ,

   /* word     T_ZISkorM_Torm  ;        */  _Skor_nom( 0.020 ) ,
...
/*------------  Для Модуля Задания на Скорость  -------------------*/
   ...
   /* word     Mashtab_ZSkA ;           */  (w)( 0.00 * 256.) ,             //35

   /* struct SELSIN_ust  Selsin[_Sels_Num] */ {{ _Grad( 0 ), 0x0000 },
                                               { _Grad( 0 ), 0x0000 }},
   /* struct PDF_ust  PDF[_PDF_Num]     */ {{ 600, _Grad( 0 ), _PDF_delitel }},
   ...
   /* word     Z_SkMax_P ;              */  _Skor_nom( 1.00 )  ,            //42

   /* word     Z_SkMax_M ;              */  _Skor_nom( 0.00 ) ,             //43

   /* word     Z_SkMin ;                */  _Skor_nom ( 0.02 ) ,                //44
   ...
   /* word     KRSP ;                   */  (w)( 3.50 * 256.), // Kпс = Kпн*2/32 = Kпн*0.0625
                                                               // Кпн = Кпс/0.0625 = 1/0.0625=16
   /* word     KRSI  ;                  */  120 , // "мсек"   // Киc = 2/32*3.3 / Т
                                                            // Кис = 0.206 / T => T=0.206/Кис=..мс
   /* word     KRSPIP ;                 */  (w)( 0.00 * 256.) , // Kпипс = Kпипн*2/32 = Kпипн*0.0625
                                                                // Кпипн = Кпипс/0.0625
  /*----  Для Регулятора Тока  ----*/

   /* word     OuRSMaxMost1 ;           */  1.00 * _Id_Nom  ,               // 2

   /* word     OuRSMaxMost2 ;           */ (w)(int)(-1.00 * _Id_Nom)  ,     // 3

   /* word     OuRSMax2Most1 ;          */  0.10 * _Id_Nom ,               // 4

   /* word     OuRSMax2Most2 ;          */ (w)(int)(-0.10 * _Id_Nom) ,     // 5

   /* word     INN0;                    */  0.08 * _Id_Nom ,                // 7
#ifndef _KTE_GD
   /* word     IZMAX;                   */  1.00 * _Id_Nom ,                // 1
#endif
   /* word     Temp_RT_P;               */  30 * _Id_Nom200 ,                //10

   /* word     Temp_RT_M;               */  30 * _Id_Nom200 ,                //11

   /* word     RTMIN;                   */  0x0091       , //_Grad(   0 ) , //0x0300         ,

   /* word     RTMAX;                   */  0x3A7B       , //_Grad( 127 ) ,//_Grad(  90 ) , //0x3A55         ,
#ifndef _KTE_GD
   /* word     KRTDI;                   */  112u,
...
//---------------  Для Защит -------------------
   ...
   /* word     Ostanov_skor ;           */  _Skor_nom ( 0.05 ) ,            //62

   /* word     Ostanov_time ;           */  _Sec( 0.2 ) ,                   //63

   /* word     Stoyanka_Tok ;           */  _Id_nom  ( 1.10 ) , //_Id_nom  ( 2.00 ) , //65

   /* word     Stoyanka_Eds  ;          */  _Skor_nom ( 0.05 ) , //_EDS_nom ( 0.07 ) , //64
  ...
  /*----  Для Обработки Дискретных Входов -----*/
   ...
   /* byte     Pi0_Normal ;          ы  */  0xCE, //0x8E, //0x96,
                                            // Есть KF, KFV - 0xB3, Нет KFV - 0xB7, нет KF - 0xF7.
   /* byte     Pi1_Normal ;             */  0xCF, /*0xB7,*/ //0x83, //0x8F, //0xBF ,
#ifdef _Pi2_
   ...
   /*    union  CAN_CONFIG config ; */        _can_cfg_bt_125k | _can_cfg_vkl | _can_cfg_master,
   ...
   /*  } ;                          */
//#ifdef _CAN_MASTER
     { 0x01, 0x04, _can_cfg_bt_125k /*| _can_cfg_master | _can_cfg_vkl*/, _bt0_100k, _bt1_100k, 0x00, 10,  60, 0 }
//#else
...
//#endif
   ...
   /* word   time_scan ;                */  _fSec(0.0), //_fSec(0.01),
   ...
   /* word   Id_short_imp ;             */  _Id_nom  ( 0.0 ) ,
//#ifdef _CAN_MASTER
   /* word   T_mConnect_cfg ;           */  _Sec ( 0.15 ) ,

   /* word   T_mConnect_sts ;           */  _Sec ( 0.80 ) ,

   /* byte   AddrDevice1 ;              */  0x02 ,

   /* byte   AddrDevice2 ;              */  0x03 ,

   /* byte   AddrDevice3 ;              */  0x04 ,

   /* byte   AddrDevice4 ;              */  0x05 ,

   /* byte   AddrDevice5 ;              */  0x06 ,

   /* byte   AddrDevice6 ;              */  0x07 ,
...
/*    }                                 */  },
...
#endif
   /* union Config2  Cfg2 ;             */  { 0 } ,

   /* word   Password ;                 */  0 ,
#ifndef _ZI_30min
   /* word   T_ZISkorP_Fors ;           */  _Skor_nom( 1.50 ) ,
#else
   ...
   /* word   Stoyanka_Time_Predupr ;    */  _Sec( 5.0 ) ,
