...
else
{
 Set_ZI ( &zi_rs , ZISkor ) ; // Инициализация всего ЗИРС .
      ...
      ziskor = Z_Skor ;
         ...
         // Проверка Задания Скорости на уставочные ограничения .
...
//------------------
 ...
 ZISkor = zi_rs.out ;
#else
 ziskor = zi_rs.out - OIRT + _or.ZIS_Nachal ;  // Вычитаем из ЗИРС выход РТ .
 if ( (sw)ziskor < 0  )  ziskor = 0 ;          // Не пускаем выход ЗИРС в отрицательную полярность .

           if ( (sw)ziskor >= (sw)Z_Skor )       ZISkor = Z_Skor ; // убираем начальное задание при Пуске0.1 .
      else if ( (sw)ziskor >= (sw)_r.Z_SkMax_P ) ZISkor = _r.Z_SkMax_P ;
      else if ( (sw)ziskor <  (sw)_r.Z_SkMax_M ) ZISkor = _r.Z_SkMax_M ;
      else                                       ZISkor = ziskor ;
#endif
...
// ------------- Реализация режима "ОСТАНОВ" ---------------
 ...
 //ДДДДДДДДД Расчет ПИП-части. ДДДДДДДДДДДДД
  ...
  #else
    lbx = lbx * _Grad( 1 ) / _Skor_nom(0.01) << 8 ; // коэфф."Ki=x*65536".
  #endif
#endif
...
//ДДДДДДДДДДД Расчет ошибки.  ДДДДДДДДДДДДДД
    ...
    ax = ZISkor - Skor ;
  #else
    ax = Skor - ZISkor ; // "Переворачиваем" вычисление ошибки , т.к. регулятор действует на Угол Управления
  #endif
    Delta_Sk = ax ;

  #ifdef _UPTF
      // Обход интегратора сделан как в якорном РТ , по достижению Альфа-макс и Альфа-мин . При нахождении на упорах
      // 20-грд дельты ЗИ угла , пока что не делаем обход интегратора .
      // Обнуление флагов здесь не делаем , ожидаем обнуления перед следующим вызовом ЗИ Угла ...
      if ( (sw)Delta_Sk >= 0 )
        {
          if ( S.flg._.Fmax == 1 /*|| S.flg._.Fdmax == 1*/ )  goto v4 ;
        }
      else
        {
          if ( S.flg._.Fmin == 1 /*|| S.flg._.Fdmin == 1*/ )  goto v4 ;
        }
  #endif
...
#endif
 //           При реверсе , не требуется
         Prz._.torm_zi_p = Prz._.torm_zi_m = 0 , Ogr_zi_rev = 0 ;
...
//   Проверка ограничений на возрастание или снижение интегратора.
 ...
 //ДДДДДДДДДДД
  ...
  #else
    lax = (lw)( (d)_Grad( 1 ) / (d)_Skor_nom(0.01) * time_Integr/1000 *  65535.) / lax ;
  #endif
#endif
 ...
 //ДДДДДДДДДДД
  ...
  #else
    lax = lax * _Grad( 1 ) / _Skor_nom(0.01) << 8 ; // коэфф."Ki=x*65536".
  #endif
#endif
    lax &= 0x7ffffffful ;   //защита от "-" коэфф.
    //--------------------------------------------------
    ...
    p1:       OuRegS_dop_kod = 0x07ff , Prz._.int_p = 1 ; // +512*4 дискрет .
          ...
          if ( (slw)lax < ((slw)_r.RTMIN << 16) )
            {
    p2:       OuRegS_dop_kod = (lw)_r.RTMIN , Prz._.int_m = 1 ;
            }  // не подпускаем к краю сетки, чтобы можно было контролир.переполн.
          else if ( (slw)lax >= ((slw)_r.RTMAX << 16) )
            {
    p1:       OuRegS_dop_kod = (lw)_r.RTMAX  , Prz._.int_p = 1 ;
            }
  #endif
#else
          if ( (slw)lax < (slw)0xfe000000 )
    ...
    p1:       OuRegS_dop_kod = 0x01ff , Prz._.int_p = 1 ; // +512 дискрет .
     ...
     ORT = OuRegS_dop_kod ; // Выход РС передаём на вход линеаризации .
#else
 #ifdef _Rev_Pola
