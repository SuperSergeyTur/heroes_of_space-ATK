...
//#define  _LABOR_STEND    // Отличия набора плат и оборудования Лабораторного Стенда АТК от серийного изделия .
...
#define   _ATK_SET         // АТК - АР1 Сетевой инвертор .
 ...
 #else
  #define _PDF_FiltrNumb     0x0 // На синхронной скорости 1000 об/мин ПДФ 1000 имп/об на стенде выдаёт 166 кГц ,
  #define _PDF_FiltrNumbINX  0x0 // соответственно , фильтровать такой сигнал нельзя .
 #endif
...
//#define _Dat_Temp_Max 12 // Для алгоритма аналогового измерения температуры каждого охладителя в каждом силовом шкафу .
...
//#define  _IU_60_Qubler   // Схема Кюблера
//---
// ЦАП PPG5 или сумма встроенного возбудителя на порту P1.22 . Запайка #4 должна быть 2-3 .
//#define  _Vozb_KTE       // Главная плата управления выдает ИУ возбудителем . Запайка #4 должна быть 2-3 .
//#define  _SIFUV_1Faza  1          // Возб. 1-фазный.
//#define  _SIFUV_1Faza_PoluUpr_TT  // Возб. 1-фазный, полууправляемый, ДТ ТТ .
//#define  _RNV            // Регулятор напряжения возбуждения вместо РТВ .
//---
...
#define     bi_xQ1  bi_AK1 // Отключение Q1 , когда его б/к включен .
#define     bi_xQ2  bi_AK2 // Отключение Q2 , когда его б/к включен .
//---
//  #ifndef  _LABOR_STEND    // На синхронной скорости 1000 об/мин ПДФ 1000 имп/об на стенде выдаёт 166 кГц
...
//---
//#define  _ShK_own        // ШК собственный для каждого АТК , а не один . Для контроля Дверей ШК по сети .
#define    _ShK_Rele_Vedom // В ШК приходят Авар , предупр , Готовн и Работа и Ведущего и Ведомого АТК .
//#define  _CAN_Pusk       // Ведущий АТК передаёт Пуск в Ведомый АТК по сети .
//---
