...
#else
      // АТК - Сетевой инвертор АР1 :
      //---
      // Изначально АР1 находится в режиме Инвертора ( задаётся при подаче Пуска ) .
      // Дальнейшее переключение режима инверт(выпрям) происходит после реверса в файле RT.C .
      //---
      // В АТК , в отличие от КТЕ , INN при увеличении скорости растёт .
      // Однако , используем только измерение граничного тока , рассчеты его не производим ...
      //---
      EDS = EDS_dop_kod = ED1 = 0 ; // Обнуляем все рассчеты ЭДС .
#endif
