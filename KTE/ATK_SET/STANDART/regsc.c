...
#include "regsc_proizv.c"
 ...
 // Задатчик Интенсивности
      ...
      //NEW_ZI zi_rs.in  = ZISkor ; //,  zip = &zi_rs ;
      zi_rs.set_in( ZISkor ) ; //NEW_ZI

if ( Isp._.ZIRS_shunt == 0 ) // Если есть ЗИРC :
{
 //NEW_ZI Zad_Intens ( &zi_rs ) ;
    zi_rs.zad_intens() ; //NEW_ZI
}
else
{
 //NEW_ZI Set_ZI ( &zi_rs , ZISkor ) ; // Инициализация всего ЗИРС .
    zi_rs.set_zi( ZISkor ) ; //NEW_ZI
}

//---------------- торможение ЗИ при реверсе
 if ( S.flg._.Revers == 1 )
   {
     if ( Prz._.torm_zi_p == 1 && (sw)zi_rs.out > (sw)Ogr_zi_rev ) zi_rs.set_zi( Ogr_zi_rev ) ;//NEW_ZI  zi_rs.out = Ogr_zi_rev ;

     if ( Prz._.torm_zi_m == 1 && (sw)zi_rs.out < (sw)Ogr_zi_rev ) zi_rs.set_zi( Ogr_zi_rev ) ;//NEW_ZI  zi_rs.out = Ogr_zi_rev ;
   }
//------------------
...
//   Проверка ограничений на возрастание или снижение интегратора.
 ...
 //ДДДДДДДДДДД
    ...
    lax = AddZSkorProizv( 1, ZISkor, lax );
...
void  Fors_Temp ( void )
{
  ...
  if ( (bi_Fors_Temp == 0 &&
    ...
    {
      zi_rs.pTemp_p_razg = &_r.T_ZISkorP_Razg ;  //NEW_ZI
      zi_rs.pTemp_p_torm = &_r.T_ZISkorP_Torm ;  //NEW_ZI
    #ifndef _ZI_4TEMPA
      zi_rs.pTemp_m_razg = &_r.T_ZISkorP_Razg ;  //NEW_ZI
      zi_rs.pTemp_m_torm = &_r.T_ZISkorP_Torm ;  //NEW_ZI
    #else
      zi_rs.pTemp_m_razg = &_r.T_ZISkorM_Razg ;  //NEW_ZI
      zi_rs.pTemp_m_torm = &_r.T_ZISkorM_Torm ;  //NEW_ZI
    #endif
  ...
  // Форсированный темп ( Ревизию не проверяем , у нее приоритет меньше ) :
    ...
    {
      zi_rs.pTemp_p_razg = &_r.T_ZISkorP_Fors ;  //NEW_ZI
      zi_rs.pTemp_p_torm = &_r.T_ZISkorP_Fors ;  //NEW_ZI
      zi_rs.pTemp_m_razg = &_r.T_ZISkorP_Fors ;  //NEW_ZI
      zi_rs.pTemp_m_torm = &_r.T_ZISkorP_Fors ;  //NEW_ZI
  ...
  // Темп ЗИС для Ревизии ( Перед этим проверен Штатный темп и Форсированный темп ) :
    ...
    {
      zi_rs.pTemp_p_razg = &_or.T_ZISkor_Reviz ;  //NEW_ZI
      zi_rs.pTemp_p_torm = &_or.T_ZISkor_Reviz ;  //NEW_ZI
      zi_rs.pTemp_m_razg = &_or.T_ZISkor_Reviz ;  //NEW_ZI
      zi_rs.pTemp_m_torm = &_or.T_ZISkor_Reviz ;  //NEW_ZI
