...
#define    bo_MB1_Otkl           Po0_c._.n3 // Авар.откл. Масляного Выключателя
#define    bo_MB1_Razr           Po0_c._.n4
#define    bo_MT                 Po0_c._.n5
...
#define    bi_1B1                Pi0_f._.n2 // Не обрабатывается , определяем срабатывание 1В методом исключения .
#define    bi_2B1                Pi0_f._.n3
#define    bi_3B1                Pi0_f._.n4
#define    bi_4B1                Pi0_f._.n5
...
//--------------------------------------------------------------------------
 // Не перепутать ! Номер INT и номер в Порту 1 - НЕ совпадают :
  #ifdef   _SIFU2_
#define    bi_KF_VKask           1 //Pi1_f._.n4 - ДЗВА2 - INT4
#define    bi_KFV_VKask          1 //Pi1_f._.n5 - ДЗВК2 - INT3
  #else
...
//--------------------------------------------------------------------------
// Все герконы заходят на прерывание - вход 1В , а дифференцируются по месту
// возникновения ( ШС1 , ШС2 , ШС3 или ШС4 ) при помощи "bi_2B1" , "bi_3B1", "bi_4B1" .
// При этом "bi_1B1" нам не требуется ...
  #ifdef    bi_2B1
#define    mTest_Gercon2B1()    (((b)(LPC_GPIO2->PIN >> 19) & 0x01) == 0) // bi_2B1 - Pi0_f._.n3
  #endif
  #ifdef    bi_3B1
#define    mTest_Gercon3B1()    (((b)(LPC_GPIO2->PIN >> 20) & 0x01) == 0) // bi_3B1 - Pi0_f._.n4
  #endif
  #ifdef    bi_4B1
#define    mTest_Gercon4B1()    (((b)(LPC_GPIO2->PIN >> 21) & 0x01) == 0) // bi_4B1 - Pi0_f._.n5
  #endif
//--------------------------------------------------------------------------
...
// Для раздельной работы 12-ти пульсной схемы :
//#define  bi_Vedusch            PiS_f[]._.n // ШС1,ШС2
//#define  bi_Both               PiS_f[]._.n // ШС1,ШС2 + ШС3,ШС4
//#define  bi_Vedom              PiS_f[]._.n //           ШС3,ШС4

//-------------------
#define  _ACh0_txt      "n0 -Id1       "
#define  _ACh1_txt      "n1 -Id1-средн."
#define  _ACh2_txt      "n2 -резерв    "
#define  _ACh3_txt      "n3 -Тахоген.  "
#define  _ACh4_txt      "n4 -N#        "
#define  _ACh5_txt      "n5 -Ud        "
#define  _ACh6_txt      "n6 -Id2       "
#define  _ACh7_txt      "n7 -Id2-средн."
...
#define    Id2_mg_ach           _ar.n06_ach
#define    Id2_sr_ach           _ar.n07_ach
...
_x_far const  struct  oBlok_Ustavok  _oc   =
 ...
 /*  word   KRTPRI_2_ ;            */ 15*64 , // "мсек" // Киc = 91/2*3.3 / Т
 /*  word   KRTNI_2_  ;            */ 150*64, // "мсек" // Киc = 91/2*3.3 / Т
 /*  word   KRTP_2_   ;            */ (w)( 0.21 * 256.) , // Kпс = Kпн*91/2 = Kпн*45
 /*  word   INN0_2_   ;            */ 0.08 * _Id_Nom ,

 /*  word   INN0_dvig ;            */ 0.04 * _Id_Nom ,
 /*  word   RVT_INN0  ;            */ _Grad( 0.0 ) ,
 ...
 /*  word   Is_tokoraspred ;       */  _Id_nom(0.30) ,
 /*  word   dIs_ControlBegin       */  _Id_nom(0.50) ,

 /*   word   Mashtab_Id_sum ;      */ (w)( 1.151 * 256.) ,
 /*   word   Id_sum_max ;          */ _Id_nom ( 2.65 ) ,

 /*   word   KINN ;                */ (w)( 0.80 * 256.) ,
...
_x_far const struct SBlok_Ustavok _sc =
  {
      //  Уставки номиналов: 1дискрета = 1А, 1В.
                                       {   {  0, 0 },
 /* struct  {  word  oe , fe ;  } Id ; */     { _Id_nom( 1.0 ), 5000 },
 /* struct  {  word  oe , fe ;  } Ud ; */     { _Ud_nom( 1.0 ), 930 },
 /* struct  {  word  oe , fe ;  } Iv ; */     { _Iv_nom( 1.0 ), 200*10 },
