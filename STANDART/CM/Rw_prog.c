  ...
  switch( Label )
  {
      ...
      case m0:
             if( TestCompleted_Output( ) != 0 )
             {
                 ...
                 crs -= size;
                 ax = (w)(flash_begin>>16);
                 bh = Hb( ax ); // старший байт старшего слова начального адреса
                 output_c( bh );
                 crs -= bh;
                 bh = Lb( ax ); // младший байт старшего слова начального адреса
                 output_c( bh );
                 crs -= bh;
                 ax = (w)flash_begin;
                 bh = Hb( ax ); // старший байт младшего слова начального адреса
                 output_c( bh );
                 crs -= bh;
                 bh = Lb( ax ); // младший байт младшего слова начального адреса
                 output_c( bh );
                 crs -= bh;
                 ax = (w)(flash_end>>16);
                 bh = Hb( ax ); // старший байт старшего слова конечного адреса
                 output_c( bh );
                 crs -= bh;
                 bh = Lb( ax ); // младший байт старшего слова конечного адреса
                 output_c( bh );
                 crs -= bh;
                 ax = (w)flash_end;
                 bh = Hb( ax ); // старший байт младшего слова конечого адреса
                 output_c( bh );
                 crs -= bh;
                 bh = Lb( ax ); // младший байт младшего слова конечого адреса
                 output_c( bh );
                 crs -= bh;
      ...
      case wr1:
             ...
             Label++;
      case wr2:
             ax = xSave_into_i2c( 2, (b*)&SizeFile.all, s_addr, flash_address ); // запись 2-х байт размера файла
      ...
      case wr4:
             ...
             Label++;
      case wr5:
             ax = xSave_into_i2c( count, (b*)&m_buff, s_addr, flash_address );
             if( ax == 4 ) break;  // пока = 4 - продолжается запись
             Mon.i2c_busy = 0;  // Устанавливаем признак, что теперь шина свободна.
             if( ax == 1 )
             {
      ...
      case rd0:
             ...
             Label++;
      case rd1:
             ax = xLoad_from_i2c( 2, s_addr, (b*)&SizeFile.all, flash_address ); // считываем размер файла
      ...
      case rd3:
             ...
             Label++;
      case rd4:
             ax = xLoad_from_i2c( size, s_addr, (b*)&m_buff, flash_address ); // считываем контрольный код
             if( ax == 4 ) break;  // пока = 4 - продолжается чтение
             Mon.i2c_busy = 0;  // Устанавливаем признак, что теперь шина свободна.
             if( ax == 1 )
             {
      ...
      // очистка
      case cl0:
             SizeFile.all = flash_end - s_addr + 1;  // вычисляем сколько байт осталось до конечного адреса
