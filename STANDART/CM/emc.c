...
//---------------   Конфигурация адресного пространства флэш   ------------------------
    LPC_EMC->StaticConfig1   = 0x81;
    LPC_EMC->StaticWaitWen1  = 2;//0;
    LPC_EMC->StaticWaitOen1  = 2;//0; // должен быть 0 - иначе WaitRd надо увеличивать
    LPC_EMC->StaticWaitRd1   = 4;//0x2; // 1 - потому что с 0 не работает
    LPC_EMC->StaticWaitPage1 = 0;
    LPC_EMC->StaticWaitWr1   = 4;//0;
    LPC_EMC->StaticWaitTurn1 = 0;
//-------------------------------------------------------------------------------------

//-------------   Конфигурация адресного пространства сетевой платы   -----------------
//#ifndef _ETHERNET // Для наездника PROFIBUS
    LPC_EMC->StaticConfig2   = 0x80;   // для начала настраиваем на 8-bit
//#else           // Для наездника ETHERNET MODBUS CAN
//    LPC_EMC->StaticConfig2   = 0x81;   // для начала настраиваем на 8-bit
//#endif
...
//------   проверка   -----
   ...
   //Для ЭОЗУ не надо обнулять память для проверки записи во флеш
   if (Region != _EMC_CS1)
   {
