...
void I2C_Init(uint8_t i2cId, uint32_t clockrate)
{
	...
	LPC_I2C_TypeDef* I2Cx = I2C_GetPointer((en_I2C_unitId)i2cId);
 ...
 * corresponding.
 **********************************************************************/
Status I2C_MasterTransferData(uint8_t i2cId, I2C_M_SETUP_Type *TransferCfg,																	I2C_TRANSFER_OPT_Type Opt)
{
	LPC_I2C_TypeDef* I2Cx = I2C_GetPointer((en_I2C_unitId)i2cId);
...
retry:
		...
		if ((TransferCfg->tx_length != 0) && (TransferCfg->tx_data != NULL))
		{
                  ...
                  while (TransferCfg->tx_count < TransferCfg->tx_length)
                  {
                    ...
                    //Не использую for т.к. оптимизация его может выкинуть.
                    //Временная задержка на запоминание данных 
                    txdat++;
