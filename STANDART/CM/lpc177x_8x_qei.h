...
/* QEI Reset position counter on Index Signal Option */
/** Reset position counter on Index: None */
#define QEI_RESPI_NONE         ((uint32_t)(0))
/** Reset position counter on Index: Enable */
#define QEI_RESPI_EN           ((uint32_t)(1))
...
typedef struct
{
   	...
   	uint32_t InvertIndex		:1; 	/**< Invert Index Option:
   										...
   										*/
         uint32_t CRESPI                 :1;     /**< Reset counter on index Option:
                                                        - QEI_RESPI_NONE: reset on index is off
                                                         - QEI_RESPI_EN:   reset on index is on
   										*/
         uint32_t NC                     :11;          //reserve
         
         uint32_t INXGATE                :4;          //INXGATE
