...
//--------------------------------------------------
             /* сброс сигнала SCL. */
#define    mSCL_clr()    PDR3 &= ~0x40
             /*   установка сигнала SCL с ожиданием его установки,
              * если его кто-то притормозил.
              */
#define    mSCL_set()    PDR3 |=  0x40  ;

             /* тестирование линии SDA */
#define    mSDA()      (( PDR3  &  0x80 ) != 0 )
             /* сброс сигнала SDA. */
#define    mSDA_clr()    PDR3 &= ~0x80
             /* установка сигнала SDA. */
#define    mSDA_set()    PDR3 |=  0x80

#define    mSDA_in()    DDR3 &= ~0x80

#define    mSDA_out()   DDR3 |= 0x80

#define    mSCL_SDA_clr()   PDR3 &= ~0xC0   // 06.02.2015 14:59
