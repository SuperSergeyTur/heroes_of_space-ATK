...
void  Init_CPU_Ports ( void )
{
  ...
  DDR1 = 0x78 ;  // 0/0  P1.0        - вход задания спец.режимов
  PDR1 = 0x08;   // 0/0  P1.1        - вход задания спец.режимов
                 // 0/0  P1.2        - вход задания спец.режимов
                 // 1/1  P1.3        - выходной Q1-сигнал.
                 // 1/0  P1.4        - не используется
                 // 1/0  P1.5        - не используется
                 // 1/0  P1.6        - USEL.
                 // 0/0  P1.7        - ОСТАНОВ.
  ...
  PDR5_P50 = 0; //  PPG2/3 - частотное заполнение ИУ моста (и.п.'0').
  PDR5_P51 = 0;               //  PPG0/4 - частотное заполнение ЦАП.
