 ...
 */
byte ToggleBit(bool Clr,byte State)
{
  static word ToggleTime;
  static bool B_State;
  byte Out = State;
  if (Clr)
  {
    ToggleTime = Timer1_fSec;
    B_State = false;       
  }
  else if ((u)((w)(Timer1_fSec - ToggleTime)) > _Sec(0.7))
  {
    B_State = !B_State;
    ToggleTime = Timer1_fSec;
  }
...
//                      output_c ( BckSp ) ;
              ...
              if(( TestCompleted_Output() ) &&((u)((w)(Timer1_Ovr - m_time)) > _Sec(0.1)))
              {
                Label = 21;
                m_time = Timer1_Ovr;
              }
